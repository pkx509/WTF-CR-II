...
{
    [Authorize]
    public class ProductShapeController : BaseApiController
    {
        private readonly IProductShapeService _Service;
        private readonly IMessageService _messageService;

        public ProductShapeController(IProductShapeService service,
        ...
        [HttpGet]
        [Route("api/ProductShape/getbyid")]
        // [ResponseType(typeof(ProductShape))]
        public async Task<ApiResponseMessage> getbyid(Guid id)
        {
            try
            {
                ...
                MasterModel.Products.ProductShape result = await Task.Run(() =>
                {
        ...
        [HttpGet]
        [Route("api/ProductShape/get")]
        //[ResponseType(typeof(List<ProductShape>))]
        public async Task<ApiResponseMessage> get(string keyword)
        {
            try
            {
                int _totalRecord = 0;
                HttpCustomHeaders _header = ApiHelpers.Response(Request);
                System.Collections.Generic.List<MasterModel.Products.ProductShape> result = await Task.Run(() =>
                {
