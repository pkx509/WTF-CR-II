...
{
    [Authorize]
    public class ProductSubStatusController : BaseApiController
    {
        private readonly IProductSubStatusService _Service;
        private readonly IMessageService _messageService;

        public ProductSubStatusController(IProductSubStatusService service,
        ...
        [HttpGet]
        [Route("api/ProductSubStatus/getbyid")]
        //[ResponseType(typeof(ProductSubStatus))]
        public async Task<ApiResponseMessage> getbyid(Guid id)
        {
            try
            {
                ...
                MasterModel.Utility.ProductSubStatus result = await Task.Run(() =>
                {
        ...
        [HttpGet]
        [Route("api/ProductSubStatus/get")]
        //[ResponseType(typeof(List<ProductSubStatus>))]
        public async Task<ApiResponseMessage> get(string keyword)
        {
            try
            {
                int _totalRecord = 0;
                HttpCustomHeaders _header = ApiHelpers.Response(Request);
                System.Collections.Generic.List<MasterModel.Utility.ProductSubStatus> result = await Task.Run(() =>
                {
