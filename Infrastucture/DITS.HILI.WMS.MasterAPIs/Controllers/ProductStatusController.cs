...
namespace DITS.HILI.WMS.MasterAPIs.Controllers
{
    ...
    public class ProductStatusController : BaseApiController
    {
        ...
        [HttpGet]
        [Route("api/productstatus/getbyid")]
        //[ResponseType(typeof(ProductStatus))]
        public async Task<ApiResponseMessage> getbyid(Guid id)
        {
            try
            {
                ...
                MasterModel.Utility.ProductStatus result = await Task.Run(() =>
                {
        ...
        [HttpGet]
        [Route("api/productstatus/get")]
        public async Task<ApiResponseMessage> get(string keyword, int? pageIndex, int? pageSize, Guid? documentTypeID)
        {
            try
            {
                int _totalRecord = 0;
                HttpCustomHeaders _header = ApiHelpers.Response(Request);
                System.Collections.Generic.List<MasterModel.Utility.ProductStatus> result = await Task.Run(() =>
                {
                    return _Service.Get(keyword, documentTypeID, out _totalRecord, _header.PageIndex, _header.PageSize);
                });
        ...
        [HttpGet]
        [Route("api/productstatus/getbydocumenttypeid")]
        //[ResponseType(typeof(ProductStatus))]
        public async Task<ApiResponseMessage> getbydocumenttypeid(Guid documentTypeid)
        {
            try
            {
                int _totalRecord = 0;
                ...
                System.Collections.Generic.List<MasterModel.Utility.ProductStatus> result = await Task.Run(() =>
                {
                    return _Service.GetByDocuemtnType(documentTypeid);
                });
