...
namespace DITS.HILI.WMS.MasterAPIs.Controllers
{
    ...
    public class ProductBrandController : BaseApiController
    {
        ...
        [HttpGet]
        [Route("api/ProductBrand/getbyid")]
        //[ResponseType(typeof(ProductBrand))]
        public async Task<ApiResponseMessage> getbyid(Guid id)
        {
            try
            {
                ...
                MasterModel.Products.ProductBrand result = await Task.Run(() =>
                {
        ...
        [HttpGet]
        [Route("api/ProductBrand/get")]
        //[ResponseType(typeof(List<ProductBrand>))]
        public async Task<ApiResponseMessage> get(string keyword)
        {
            try
            {
                int _totalRecord = 0;
                HttpCustomHeaders _header = ApiHelpers.Response(Request);
                System.Collections.Generic.List<MasterModel.Products.ProductBrand> result = await Task.Run(() =>
                {
