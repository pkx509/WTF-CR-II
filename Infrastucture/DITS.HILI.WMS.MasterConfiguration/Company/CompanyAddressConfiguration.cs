...
{
    public class CompanyAddressConfiguration : System.Data.Entity.ModelConfiguration.EntityTypeConfiguration<CompanyAddess>
    {
        ...
        {
            ToTable(schema + ".sys_company_address");
            HasKey(x => new { x.CompanyID, x.Sequence });

            Property(x => x.CompanyID).IsRequired().HasColumnName("CompanyID").HasColumnType("uniqueidentifier").HasDatabaseGeneratedOption(DatabaseGeneratedOption.None);
            ...
            HasRequired(x => x.Company).WithMany(x => x.AddessCollection).HasForeignKey(x => x.CompanyID).WillCascadeOnDelete(false);
