...
{
    public partial class frmDock : BaseUIPage
    {
        ...
        public object BindData(string oDataKeyId)
        {
            ...
            Core.Domain.ApiResponseMessage apiResp = ClientService.Queue.QueueClient.GetDockAll(status, txtSearchDock.Text).Result;
            List<QueueDock> data = new List<QueueDock>();
            if (apiResp.ResponseCode == "0")
            {
                total = apiResp.Totals;
                data = apiResp.Get<List<QueueDock>>();
            }
        ...
        private void GetAddEditForm(string oDataKeyId)
        {
            ...
            string strTitle = (oDataKeyId == "new") ? GetResource("ADD_NEW") + " " + GetResource("QUEUEDOCK") : GetResource("EDIT") + " " + GetResource("QUEUEDOCK");
            WindowShow.Show(this, strTitle, "Create Dock", "Editor/frmDockAddEdit.aspx?oDataKeyId=" + oDataKeyId, iconWindows, 450, 200);
        }
        ...
        protected void CommandClick(object sender, DirectEventArgs e)
        {
            ...
            {
                Core.Domain.ApiResponseMessage data = ClientService.Queue.QueueClient.GetDockById(Id).Result; 
                QueueDock _data = data.Get<QueueDock>();
                if (_data == null)
                {
                    return;
                }
                System.Threading.Tasks.Task<Core.Domain.ApiResponseMessage> apiResp = ClientService.Queue.QueueClient.RemoveDock(_data);
