...
{
    public partial class frmShipFrom : BaseUIPage
    {
        ...
        public object BindData(string oDataKeyId)
        {
            ...
            Core.Domain.ApiResponseMessage apiResp = ClientService.Queue.QueueClient.GetShippingFromAll(status, txtCompanyName.Text, txtCompanyShortName.Text).Result;
            List<ShippingFrom> data = new List<ShippingFrom>();
            if (apiResp.ResponseCode == "0")
            {
                total = apiResp.Totals;
                data = apiResp.Get<List<ShippingFrom>>();
            }
        ...
        private void GetAddEditForm(string oDataKeyId)
        {
            ...
            string strTitle = (oDataKeyId == "new") ? GetResource("ADD_NEW") + " " + GetResource("QUEUESHIPFROM") : GetResource("EDIT") + " " + GetResource("QUEUESHIPFROM");
            WindowShow.Show(this, strTitle, "Create From Company", "Editor/frmShippingToAddEdit.aspx?oDataKeyId=" + oDataKeyId, iconWindows, 450, 200);
        }
        ...
        protected void CommandClick(object sender, DirectEventArgs e)
        {
            ...
            {
                Core.Domain.ApiResponseMessage data = ClientService.Queue.QueueClient.GetShippingFromById(Id).Result;
                ShippingFrom _data = data.Get<ShippingFrom>();
                if (_data == null)
                {
                    return;
                }
                System.Threading.Tasks.Task<Core.Domain.ApiResponseMessage> apiResp = ClientService.Queue.QueueClient.RemoveShippingFrom(_data);
