...
namespace DITS.HILI.WMS.Web.apps.master
{
    ...
    public partial class frmPrinters : BaseUIPage
    {
        public static string ProgramCode = "P-0049";
        ...
        private void GetAddEditForm(string oDataKeyId)
        {
            ...
            string strTitle = (oDataKeyId == "new") ? GetResource("ADD_NEW") : GetResource("EDIT") + " " + GetResource("PRINTER");
            WindowShow.Show(this, strTitle, "CreatePrinter", "AddEdit/frmCreatePrinters.aspx?oDataKeyId=" + oDataKeyId, iconWindows, 450, 235);
        }
        ...
        protected void CommandClick(object sender, DirectEventArgs e)
        {
            ...
            {
                Core.Domain.ApiResponseMessage ok = ClientService.Master.PrinterClient.Remove(Id).Result;
        ...
        public object BindData(string action, Dictionary<string, object> extraParams)
        {
            ...
            Core.Domain.ApiResponseMessage apiResp = PrinterClient.Get(txtSearch.Text, prms.Page, int.Parse(cmbPageList.SelectedItem.Value)).Result;
            List<PrinterModel> data = new List<PrinterModel>();
            if (apiResp.ResponseCode == "0")
            {
                total = apiResp.Totals;
                data = apiResp.Get<List<PrinterModel>>();
            }
