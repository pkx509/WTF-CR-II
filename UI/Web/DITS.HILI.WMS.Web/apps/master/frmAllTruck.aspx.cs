...
{
    public partial class frmAllTruck : BaseUIPage
    {
        ...
        public object BindData(string action, Dictionary<string, object> extraParams)
        {
            ...
            Core.Domain.ApiResponseMessage apiResp = WarehouseClient.GetTruckType(txtSearch.Text, ckbIsActive.Checked, prms.Page, int.Parse(cmbPageList.SelectedItem.Value)).Result;
            List<TruckType> data = new List<TruckType>();
            if (apiResp.IsSuccess)
            {
                total = apiResp.Totals;
                data = apiResp.Get<List<TruckType>>();
            }
        ...
        private void GetAddEditForm(string oDataKeyId)
        {
            ...
            string strTitle = (oDataKeyId == "new") ? GetResource("ADD_NEW") : GetResource("EDIT") + " " + GetResource("TRUCK");
            WindowShow.Show(this, strTitle, "CreateTruck", "AddEdit/frmCreateTruck.aspx?oDataKeyId=" + oDataKeyId, iconWindows, 350, 250);
        }
        ...
        protected void CommandClick(object sender, DirectEventArgs e)
        {
            ...
            {
                Core.Domain.ApiResponseMessage ok = ClientService.Master.WarehouseClient.RemoveTruckType(Id).Result;
                if (ok.IsSuccess)
                {
