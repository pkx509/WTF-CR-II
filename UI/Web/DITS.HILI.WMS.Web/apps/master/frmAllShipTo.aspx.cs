...
{
    public partial class frmAllShipTo : BaseUIPage
    {
        ...
        public object BindData(string action, Dictionary<string, object> extraParams)
        {
            ...
            Core.Domain.ApiResponseMessage apiResp = WarehouseClient.GetShipToRule(txtSearch.Text, ckbIsActive.Checked, prms.Page, int.Parse(cmbPageList.SelectedItem.Value)).Result;
            List<ShipToModel> data = new List<ShipToModel>();
            if (apiResp.IsSuccess)
            {
                total = apiResp.Totals;
                data = apiResp.Get<List<ShipToModel>>();
            }
        ...
        {
            string strTitle = (oDataKeyId == "new") ? GetResource("ADD_NEW") : GetResource("EDIT") + " " + GetResource("SHIPTO");
            WindowShow.Show(this, strTitle, "CreateShipTo", "AddEdit/frmCreateShipTo.aspx?oDataKeyId=" + oDataKeyId, Icon.Add, 500, 320);
        }
        ...
        protected void CommandClick(object sender, DirectEventArgs e)
        {
            ...
            {
                Core.Domain.ApiResponseMessage ok = ClientService.Master.WarehouseClient.RemoveShipTo(Id).Result;
